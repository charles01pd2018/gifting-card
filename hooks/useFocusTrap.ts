...
export interface Options {
    ...
    returnFocus?: boolean;           // whether or not to return focus to the last focused element after closing dialog box
}
...
const useFocusTrap = <T extends HTMLElement>(
    ...
    /* CONTENT */
    const { initialFocus='none', 
        tabbableElems='',
        returnFocus=false } = options;
    ...
    useEffect( () => {
        if ( isActive ) {
            ...
            return () => {
                ...
                if ( returnFocus ) lastFocusedElem.current!.focus();
